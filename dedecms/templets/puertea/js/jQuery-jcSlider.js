...
;(function($){
	$.fn.jcSlider = function(options) {
		var defaults = {
			...
			loadPath:'images/loading.gif',    //预加载loading图片路径，相对定位,如../img/gif
			setloadSize : {
			...
			setScaling : {
				width:1980,                 //设置图片宽度单位(px)
				height:600                //设置图片高度单位(px)
			}
		};
		...
		return this.each(function() {
			var $this = $(this),
				...
				Public = new function(){
					...
					this.imgNum = function(list){
						...
						var imgNumwidth=screen.availWidth-80;
						return "<dl class=\"imgNum\" style=\"position:absolute;left:"+imgNumwidth+"px;top:-200px;z-index:2; width=160px;display:none; \" >" + list + "</dl>";
					};
				...
				left=1980-firstWidth;
			$this.css({"position":"relative"}).find("ul").wrapAll(Public.hideDiv);;
			$list.css({"width":$list.find("li").length*$list.find("li").width(),"position":"absolute","top":0,"left":left,"z-index":1});
			...
			if(options.arrow){
				...
				$prev.css("left",(screen.availWidth-1005)/2)
				     .show()
				     .next("samp")
					 .css("left",(screen.availWidth+1005)/2)
					 .show();
				$prev.click(function(){
					if(_idx <= 0 ){
						_idx = imgsLen - 1;
			...
			if(options.numBtn){
				...
				$NumList.css({"width":NumWidth,"height":NumHeight,"left":screen.availWidth/2-80,"top":550})
				        .show()
						.find("dd")
			...
			function publicMode(mode,Dom,index,width,height){
				...
				switch(mode){
					case "fade":
						 ...
						 break;
					case "x":
					     $thisImg.parents("li").css("left",index*width-(1980-screen.availWidth)/2).fadeIn("fast");
						 break;
					case "y":
