...
class Archives
{
	...
	function MakeHtml()
	{
		...
		$this->TypeLink->TypeInfos['namerule'],$this->TypeLink->TypeInfos['typedir'],$this->Fields['filename']
		);
		...
		$this->Fields["arcrank"],$this->TypeLink->TypeInfos['namerule'],$this->TypeLink->TypeInfos['typedir'],$this->Fields['filename']
		);
		$this->Fields['arcurl'] = $this->Fields['fullname'] = $filenameFull;

		//对于已设置不生成HTML的文章直接返回网址
		if($this->Fields['ismake']==-1 || $this->Fields['arcrank']!=0 || $this->Fields['typeid']==0)
		{
	...
	function GetPreNext($gtype='')
	{
		...
		if(count($this->PreNext)<2)
		{
			...
			$query = "Select arc.id,arc.title,arc.typeid,arc.ismake,arc.senddate,arc.arcrank,arc.filename,
			...
			if(is_array($preRow))
			{
				...
				$preRow['namerule'],$preRow['typedir'],$preRow['filename']);
			...
			if(is_array($nextRow))
			{
				...
				$nextRow['namerule'],$nextRow['typedir'],$nextRow['filename']);
	...
	function GetPagebreakDM($totalPage,$nowPage,$aid)
	{
		...
		$PageList = "<li><a>".GetLang('total').' '.$totalPage.' '.GetLang('page')."</a></li>";
		$nPage = $nowPage-1;
		$lPage = $nowPage+1;
		if($nowPage==1)
		{
			$PageList.="<li><a href='#'>".GetLang('prepage')."</a></li>";
		}
		else
		{
			if($nPage==1)
			{
				$PageList.="<li><a href='view.php?aid=$aid'>".GetLang('prepage')."</a></li>";
			}
			else
			{
				$PageList.="<li><a href='view.php?aid=$aid&pageno=$nPage'>".GetLang('prepage')."</a></li>";
			}
		}
		...
		if($lPage <= $totalPage)
		{
			$PageList.="<li><a href='view.php?aid=$aid&pageno=$lPage'>".GetLang('nextpage')."</a></li>";
		}
		else
		{
			$PageList.= "<li><a href='#'>".GetLang('nextpage')."</a></li>";
		}
		return $PageList;
	}
	...
	function GetPagebreak($totalPage,$nowPage,$aid)
	{
		...
		else
		{
			if($nPage==1)
			{
				$PageList.="<li><a href='".$this->NameFirst.".".$this->ShortName."'>".GetLang('prepage')."</a></li>";
			}
			else
			{
				$PageList.="<li><a href='".$this->NameFirst."_".$nPage.".".$this->ShortName."'>".GetLang('prepage')."</a></li>";
			}
		}
		for($i=1;$i<=$totalPage;$i++)
		{
		...
		if($lPage <= $totalPage)
		{
			$PageList.="<li><a href='".$this->NameFirst."_".$lPage.".".$this->ShortName."'>".GetLang('nextpage')."</a></li>";
		}
		else
		{
			$PageList.= "<li><a href='#'>".GetLang('nextpage')."</a></li>";
