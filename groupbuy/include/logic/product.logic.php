...
class ProductLogic
{
		...
		function GetNewList($limit=2, $ishot = false)
	{
		...
		$sql = 'SELECT `id`,`name`,`price`,`nowprice`,`img`, `linkid`,`virtualnum`,(`virtualnum`+`sells_count`) AS `sells_count`,`begintime`,`intro`,`type`,`overtime`,`maxnum`,`successnum`,`is_countdown`
	...
	function BuysCheck( $id, $checkIfBuyed = true, $curBuys = false, $ord_is_Paid = null, $uid = 0, $amount=0 )
	{
		...
		if ( $product['maxnum'] > 0 )
		{
			...
			if(!$surplus) return array('false' => __('该产品已经卖完了！下次请赶早'));
